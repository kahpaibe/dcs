...
from .akibaoo_settings import configure_loggers, LOG_SEARCH_PATH, LOG_ITEMS_PATH, ITEM_HTML_FOLDER_PATH, akibaoo_urls, get_id_and_image_file_name_from_url
...
# ===================================================================
# === Spider definition ===
class AkibaooSpider(scrapy.Spider):
    name = "akibaoo"
    counter_items = 0
    counter_search = 0
    ...
    def parse_search_for_products(self, response: scrapy.http.TextResponse):
        ...
        item_urls = response.xpath('//div[contains(@class, "set-group goodsInfo")]//a/@href').getall()
    ...
    def parse_product(self, response: scrapy.http.TextResponse):
        ...
        # scrape images too
        yield {"akibaoo_image_urls": image_urls}

    
    @staticmethod
    def _get_image_urls(response: scrapy.http.TextResponse) -> list[str]: # retrieve all urls of all item images
        image_urls = response.xpath('//img[@class="goodsDtlImgThumb"]/@src').getall()
        cleaned_image_urls: list[str] = []
        for img_url in image_urls:
            cleaned_image_urls.append(response.urljoin(img_url))
