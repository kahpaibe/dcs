...
class MelonbookSpider(scrapy.Spider):
    ...
    async def start(self):
        global melonbooks_urls
        for url in melonbooks_urls:
            yield scrapy.Request(url=url, callback=self.parse_events_for_products)

    def parse_events_for_products(self, response: scrapy.http.TextResponse):
        """Parse event pages.

        Yields new requests for:
            * New event pages (next page) -> self.parse_events_for_products(...)
            * Corresponding item pages -> self.parse_products(...)"""
        with open(LOG_SEARCH_PATH, "a+", encoding="utf-8") as f: # Log
            self.counter_events+=1
            f.write(f"search ({self.counter_events}): {response.url}\n")
        ...
        if next_page_button:
            ...
            yield scrapy.Request(next_page_url, callback=self.parse_events_for_products) # Follow the URL for the next page
